...
func TestWasmdExport(t *testing.T) {
	db := db.NewMemDB()
	gapp := NewFurychainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyAppOptions{}, emptyWasmOpts)
	...
	newGapp := NewFurychainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, MakeEncodingConfig(), wasm.EnableAllProposals, EmptyAppOptions{}, emptyWasmOpts)
...
func setGenesis(gapp *FurychainApp) error {
	genesisState := NewDefaultGenesisState(gapp.appCodec)
	stateBytes, err := json.MarshalIndent(genesisState, "", " ")
	if err != nil {
		return err
