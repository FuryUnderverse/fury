...
#
# Copyright Furychain All Rights Reserved
#
...
initFn(){ 
  ...
  then
    ...
    furyd init $(getArgument "moniker" $MONIKER) --chain-id Furychain
...
EOF
    ...
    # rm -f .furyd/config/genesis.json && wget https://raw.githubusercontent.com/furychain/furychain-static-files/ducphamle2-test/genesis.json -q -P .furyd/config/
...
createValidatorFn() {
  ...
  if [[ $moniker == "" ]]; then
    moniker="$USER"_"Furychain"_$(($RANDOM%100000000000))
  fi
  ...
  (echo "$PASS"; echo "$PASS") | furyd tx staking create-validator --amount $amount --pubkey $pubkey --moniker $moniker --chain-id Furychain --commission-rate $commissionRate --commission-max-rate $commissionMaxRate --commission-max-change-rate $commissionMaxChangeRate --min-self-delegation $minDelegation --gas $gas --gas-prices $gasPrices --security-contact $securityContract --identity $identity --website $website --details $details --from $user -y
}
